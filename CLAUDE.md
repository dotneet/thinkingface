...
| `scripts/` | Build-time tooling. `copy-duckdb-assets.mjs`, `check-ui.mjs` (the `check:ui` gate), and `oxc-react-compiler-loader.cjs` (below) |

Next.js 16 builds and serves with **Turbopack** — there is no webpack config, and adding one
would make `next build` refuse to run (`--webpack` opts back out, at the cost of everything
below).

The **React Compiler** runs through oxc, not Babel: `scripts/oxc-react-compiler-loader.cjs`
is wired in as a Turbopack loader from `next.config.ts`. Two things about that setup are not
negotiable, and both are explained where they live:

- it applies to the **browser graph only** (`condition: ["browser"]`). The compiler's output
  imports `react/compiler-runtime`, which reads React's *client* internals — compile a
  Server Component and the build dies prerendering it. Memoization also buys nothing on the
  server, where a component renders once;
- a module the compiler cannot handle is passed through unoptimized with a warning, never
  failed. The compiler bails out of individual functions routinely (a
  `react-hooks/exhaustive-deps` suppression, `useReactTable()`, syntax it has not
  implemented), and `react/incompatible-library` in `.oxlintrc.json` reports the library
  cases as warnings.

`reactCompiler: true` in `next.config.ts` would work too, but it drives the Babel plugin.
Interleaved cold `next build` runs on this app (M4 Max, 5 rounds, `.next` removed each time):
no compiler 8.94s, oxc loader 8.99s, Babel plugin 9.83s (fastest run of each). The oxc
loader's cost does not clear measurement noise; Babel's is about +10%.
...
- **TypeScript**: oxlint (`frontend/.oxlintrc.json`, lint) + oxfmt
  (`frontend/.oxfmtrc.json`, formatting and import sorting). Both replaced the earlier
  Biome + ESLint pair; there is no `biome.json` or `eslint.config.mjs` any more. oxlint's
  `correctness` category is stricter than what those two enforced, so `.oxlintrc.json`
  names — with the reason and the site count — every rule that is off because it fires on
  pre-existing code. One-off exceptions live at the call site as
