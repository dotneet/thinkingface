...
Direct uploads from the Web UI (adding files from the browser) go through `POST /api/v1/upload/{kind}/{ns}/{name}/{rev}` (`docs/dev/api-contract.md` §3), which streams each multipart part into either an LFS object or a git blob using the *same* routing rules as the preupload API and commits the whole set as one server-side git commit. Deleting a file from the UI is `DELETE /api/v1/edit/...`, likewise a git commit. **There is exactly one write path — git —** so history never diverges between the UI, the CLI, and Python.
