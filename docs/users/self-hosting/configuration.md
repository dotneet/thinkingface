...
| `TF_AUTH_RATE_LIMIT_PER_MIN` | Failed password attempts allowed per client address per minute (half that for one username from one address, and five times it for one username across all addresses). An IPv6 client counts as its /64. `0` disables it. | `10` | Applies to both the login endpoint and HTTP Basic auth (accepted on every route). Counted per process — with multiple replicas, the limit applies per replica, not globally. |
| `TF_TRUST_PROXY_IPS` | Identify the client from `X-Forwarded-For` instead of the connection peer, for rate limiting and for the `client_ip` in the authentication log. | `false` | Only enable this when a proxy you control sits in front — with no proxy there, the header is whatever the client typed. |
| `TF_TRUSTED_PROXY_HOPS` | How many `X-Forwarded-For` entries those proxies appended. The client is read that many places from the **right**. | `1` | Every proxy this runs behind appends rather than overwrites (GCLB, Cloud Run, nginx's `proxy_add_x_forwarded_for`), so the leftmost entry is client-supplied. Use `1` for a single proxy such as Cloud Run reached directly, `2` behind a Google load balancer (it appends the client and then the GFE). A header with fewer entries than this is ignored and the connection peer is used instead. Only read when `TF_TRUST_PROXY_IPS` is `true`. |
...
| `TF_SSH_MAX_UNAUTH_CONNS_PER_ADDR` | How many connections one source address may hold before authenticating. | `8` | **Raise this behind NAT or a bastion**, where many real clients arrive as a single address. The slot is released the moment authentication succeeds, so this bounds the pre-auth phase only. |
| `TF_SSH_MAX_UNAUTH_CONNS` | The same cap process-wide. | `512` | A backstop, set high enough that no single peer reaches it — the per-address limit above is what actually keeps one host from starving everyone else. |
