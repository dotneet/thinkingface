...
| `TF_AUTH_RATE_LIMIT_PER_MIN` | Failed password attempts allowed per client IP per minute (half that for one username from one IP, and five times it for one username across all IPs). `0` disables it. | `10` | Applies to both the login endpoint and HTTP Basic auth (accepted on every route). Counted per process — with multiple replicas, the limit applies per replica, not globally. |
