...
| `TF_ADMIN_PASSWORD` | そのシードされたアカウントのパスワード。 | `admin` | **これは変更してください。** よく知られたデフォルト値であり、`TF_PUBLIC_URL` が `https://` の間は未設定のままだと起動が拒否されます。 |
...
| `TF_ALLOW_SIGNUP` | セルフサービスでのアカウント作成を開放するかどうか。 | `true` | `false` にすると、シードされた admin だけがアカウントになります（それでも admin が他のアカウントを追加することはできます）。 |
