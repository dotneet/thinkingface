...
| `TF_AUTH_RATE_LIMIT_PER_MIN` | クライアント IP ごとに 1 分間で許容されるパスワード失敗回数（同じ IP からの同じユーザー名にはその半分、全 IP 合計での同じユーザー名にはその 5 倍）。`0` で無効化します。 | `10` | ログインエンドポイントと（すべてのルートで受け付けられる）HTTP Basic 認証の両方に適用されます。プロセス単位でカウントされるため、複数レプリカがある場合、制限はグローバルではなくレプリカごとに適用されます。 |
