...
- **レート制限**: パスワード認証の失敗（Web UI のログインフォームと、すべてのルートが受け付ける HTTP
  Basic 認証の両方）は、クライアントのアドレスごとに制限され、さらに同じアドレスからの同じユーザー名
  にはその半分、全アドレス合計での同じユーザー名にはその 5 倍の上限がかかります。これにより、あるア
  ドレスからの失敗が、別のアドレスからログインする人をロックアウトすることはなく、多数のアドレスに分
  散した推測にも上限がかかります。カウントされるのは失敗のみで、ログ
