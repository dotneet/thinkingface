...
サインアップを完全に閉じずに絞り込むための設定が 2 つあります。

- `TF_SIGNUP_EMAIL_DOMAINS` は、受け付けるメールドメインを列挙して制限します。判定は大文字小文字を
  区別せず、**完全一致**です。`example.com` を指定したインスタンスは `alice@example.com` を受け入れ、
  `alice@sub.example.com` は拒否します。ドメインが自動的にそのサブドメインすべてを許可してしまうのは、
  意図しないアカウントができて初めて気づく類の驚きだからです。拒否された場合は、受け付けるドメインが
  利用者に伝えられるため、フォームは引き続き記入可能です。
- `TF_SIGNUP_REQUIRE_APPROVAL` は、新規サインアップを「承認待ち」にします。アカウントは作成されます
  が、セッションは発行されず、管理者が **Settings → Users** で承認するまで**どの経路でも**認証され
  ません。パスワードも、アクセストークンも、SSH 鍵もです。サインアップフォームはサインインしたふりを
  せず、その旨を表示します。承認前にログインを試みると 403 `account_pending` が返ります（返るのは
  *正しい*パスワードを入力した場合だけなので、アカウントの存在が他人に漏れることはありません）。
...
変数を書き換えるのではなく別の管理者がリセットします。別の管理者がいない場合は、サーバーのコマンド
ラインから復旧します（[締め出されたインスタンスを復旧する](#recovering-a-locked-out-instance)）。
...
- **Make admin** / **Revoke admin** — サイト管理者権限を付与または剥奪します。インスタンスに
  ...
  ための手段でもあります。
- **Suspend** / **Restore** — アカウントを停止し、また復帰させます。停止されたアカウントは
  **どの経路でも**認証されません。Web UI も、HTTP Basic 経由のパスワードも、アクセストークンも、
  登録済みの SSH 鍵もです。退職者の締め出しにはこれが必要です。パスワードのリセットだけでは、
  そのアカウントのトークンと鍵は `git push` に対して有効なまま残るからです。停止は何も破壊しないので、
  復帰させれば元どおりになります。ただしセッションは失効したままです。
- **Revoke credentials** — そのアカウントが持つアクセストークンと SSH 鍵をすべて削除し、
  サインアウトさせます。停止と違い取り消せませんが、アカウント自体は動き続けます。これは
  「漏洩した疑いのある資格情報」（紛失した端末、ビルドログに出たトークン）に対する操作で、
  新しい資格情報を発行すれば業務を継続できるアカウント向けです。
- **Approve** / **Hold for approval** — アカウントをサインアップの承認待ちから解放する、あるいは
  承認待ちに戻します。これは `TF_SIGNUP_REQUIRE_APPROVAL` を有効にしたインスタンスでのみ表示されます。
  そこではセルフ登録したアカウントはどの経路でも認証できない状態から始まるため、承認こそがその人を
  実際に受け入れる操作です。承認待ちのアカウントにはバッジが付き、一覧の先頭に並び、上部に通知が
  表示されるので、誰かが気づかれないまま締め出され続けることはありません。承認待ちに戻すと、その場で
  すべての端末からサインアウトされます。停止とは独立した設定で、承認しても停止は解除されず、
  復帰させても承認済みにはなりません。

一覧には各アカウントの **Last login**（最終ログイン）も表示されます。これは*パスワード*でサインイン
した最後の時刻です。アクセストークンと SSH 鍵はそれぞれ独自の最終利用日時を持っており、意図的にこの
列を動かしません。この列が答えるのは「このアカウントはまだ誰かに使われているか」という問いであり、
毎晩動く CI のトークンはその答えを誤らせるからです。**Never** は文字どおり一度もない、という意味で、
この列が存在する前から使われていたアカウントもそう表示されます。
...
インスタンスが自分自身を締め出さないように、2 つのルールがあります。自分自身の管理者権限の解除と
自分自身の停止はできません（別の管理者に依頼してください）。そして、稼働中の管理者が最後の 1 人に
なる場合、その管理者は降格も停止もできません。停止中の管理者はこの「最後の 1 人」に数えません。
サインインできないアカウントは、存在しないアカウントと同じくインスタンスを復旧できないからです。
これらの操作はいずれも UI で隠しているだけでなく、サーバー側で強制されています。

!!! note "サイト管理はブラウザ経由のみ"
    `/api/v1/admin` のエンドポイントはセッションクッキーしか受け付けません。アクセストークンや
    HTTP Basic の資格情報は、その持ち主が管理者であっても 403 `session_required` で拒否されます。
    以前は write スコープのトークンが 1 本漏れるだけで、アカウントの作成、任意ユーザーのパスワード
    強制変更、そしてトークンを失効させても残る SSH 鍵の登録までできてしまいました。ブラウザの
    セッションを必須にすることで、自動化をその影響範囲から外しています。操作は **Settings** 配下の
    画面から行ってください。

## 締め出されたインスタンスを復旧する { #recovering-a-locked-out-instance }

ここまでの説明は、誰かがまだサインインできることを前提にしています。誰もサインインできなくなった
とき — 唯一の管理者がパスワードを忘れ、`/api/v1/admin` はブラウザのセッションしか受け付けない —
復旧はサーバー自身の上、データベースのすぐ隣で行います。

```bash
# サーバーを動かしているコンテナまたはホストで、サーバーと同じ
# DATABASE_URL を使って実行します:
thinkingface admin passwd alice     # エコーなしで 2 回入力を求めます
thinkingface admin promote alice    # サイト管理者権限を付与します
```

- **パスワードは決して引数で渡しません。** 端末からはエコーなしで 2 回読み取り（両者は一致する必要が
  あります）、端末がない場合は標準入力から読み取るため、自動化された実行でも無人で行えます。

  ```bash
  printf '%s' "$NEW_PASSWORD" | thinkingface admin passwd alice
  ```

  引数で渡したパスワードは、入力した人のシェル履歴と、そのマシンの他のすべてのユーザーから見える
  プロセス一覧に残ります。だからこそ、そのためのフラグは用意されていません。

- `admin passwd` は Web UI と同じルール（8 文字以上・72 バイト以内）を適用し、他のあらゆる
  パスワード変更と同様に、**そのアカウントをすべての端末からサインアウト**させます。アクセストークンと
  SSH 鍵はそのままです。
- `admin promote` は、管理者が 1 人もいなくなったインスタンスが管理者を取り戻すための手段です。対に
  なる「demote」はありません。権限を剥奪するのは緊急事態ではなく通常の管理業務であり、すでに画面と
  「最後の管理者」ガードが用意されているからです。
- どちらのコマンドも、誰に対して何をしたかを出力します。さらに、直したアカウントがまだ停止中または
  承認待ちの場合は警告します。どちらの状態も正しいパスワードすら拒否するため、警告がなければ作業が
  終わったと誤解してしまうからです。

ここでの認可は、デプロイ先へのシェルアクセスそのものです。このコマンドを実行できる人は、そもそも
データベースを直接読めます。それが要点で、これは動作するアカウントを必要としない唯一の復旧手段であり、
最後の手段としての「users テーブルを手で書き換える」に取って代わるものです。

## 失敗したバックグラウンドジョブ { #failed-background-jobs }

管理者には **Settings → Sync jobs**（`/settings/admin/sync-jobs`）も表示されます。push のたびに、
リポジトリのファイル一覧・検索エントリ・オブジェクトストレージへのエクスポートを作り直す
バックグラウンド処理がスケジュールされます。この処理が繰り返し失敗すると停止状態になり、
リポジトリは前回の push の内容でインデックスされたままになります。push は成功しているのに、
表示されるファイルが古いという状態です。この画面には停止したジョブと最後の試行のエラーが一覧され、
**Retry** でキューに戻せます。一覧が空であることが正常な状態です。
...
- **レート制限**: パスワード認証の失敗（Web UI のログインフォームと、すべてのルートが受け付ける HTTP
  Basic 認証の両方）は、クライアントのアドレス（IPv4 アドレス、または IPv6 の /64）ごとに制限され、
  さらに同じアドレスからの同じユーザー名にはその半分、全アドレス合計での同じユーザー名にはその 5 倍
  の上限がかかります。これにより、単一のホストやネットワークからの失敗が、別の場所からログインする人
  をロックアウトすることはなく、多数のアドレスに分散した推測にも上限がかかります。その裏返しとして、
  十分に多くのネットワークに分散した攻撃者は、この上限を使い切り、攻撃が続く間そのアカウントを制限
  状態に保つことができます。カウントされるのは失敗のみで、ログイン成功はこの制限枠を消費しません。制限にかかったリクエストは `Retry-After` ヘッダー付きの `429`
