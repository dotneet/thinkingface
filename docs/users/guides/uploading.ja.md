...
データセットかモデルかを推定し、ディレクトリ名をリポジトリ名にして、ディレクトリを 1 つの
コミットとして push します。`.env` のようにドットで始まる隠しファイル・隠しディレクトリは、
認証情報を誤って公開しないよう既定で除外されます。`tf up` は除外したパスを表示し、
`--hidden` を付けるとまとめてアップロードします。
...
が取り除かれますが、**ストレージの使用量は減りません** — そのファイルを追加したコミットはリポ
ジトリの履歴に残り続けるため、そのリビジョンではオブジェクトが引き続き取得可能である必要があり
(`git checkout`、`git lfs fetch --all`)、バケットはそのバイト列を保持し続け、あなたのネームス
ペースの使用量もそのままカウントされ続けます。自動的に回収されるのは、そもそもコミットに一度も
取り込まれなかったアップロード(中断された `tf up` や `huggingface_hub` のアップロード)だけです。
実際に容量を空ける方法については、[設定](../self-hosting/configuration.md) の
`TF_DEFAULT_STORAGE_QUOTA_BYTES` を参照してください。
...
1. LFS 以外のすべてのファイルをオブジェクトストレージの `blobs/{sha[0:2]}/{sha[2:4]}/{sha}` に公開します
