...
- `_meta.gpu.name` / `.count` / `_meta.cuda` — `torch` が入っていればそれ経由、なければ
  `nvidia-smi` から読み取ります
...
また、run の点の数・最終ステップ・開始時刻には一切カウントされないので、「この run はいくつの点
を記録したのか」がマシンの稼働時間に左右されることもありません。無効にするには
`THINKINGFACE_SYSTEM_METRICS=off` を設定します。
