...
拒否します。対象は `localhost`、`127.0.0.0/8`、`10/8`、`172.16/12`、`192.168/16`、`169.254/16` などの
リンクローカル範囲（クラウドのインスタンスメタデータへの経路です）、unspecified アドレス、`::1` です。

このチェックは意図的に 2 回行われます。webhook の作成・編集時に 1 回、そして各配信が実際に張る TCP
