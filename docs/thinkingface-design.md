...
- The card's `lineage:` block declares **lineage**. Writing the training dataset, base model, and training run lets the Sync Worker index them into `repo_lineage`, so the UI can trace both directions (upstream provenance and downstream models produced from it). The convention is in `docs/api-contract.md` §11
...
A single bucket `{project}-thinkingface` is used with two content-addressed layers that are fully independent of namespace and repository name. A human-readable layout does not exist on GCS (see `docs/content-addressed-storage-design.md` for the detailed design rationale).
...
├── wal/{storage_path}/                       # Source of truth for git history (docs/continuity-design.md)
...
- `wal/` is keyed by `repositories.storage_path` (an immutable physical location assigned when the
  ...
  moves even when a repository is renamed or transferred (`docs/repo-transfer-design.md`)
...
- Bare repositories are placed at `/data/git/{storage_path}.git` (`{storage_path}` is the same
  ...
  `docs/repo-transfer-design.md`). **The source of truth is not the disk but the WAL on GCS**
  (`docs/continuity-design.md`): a push is persisted as a WAL entry and only acknowledged after
...
- Keys are registered via the Web UI at `/settings/ssh-keys` (API in `docs/api-contract.md` §1).
...
- Authorization: repository visibility (public/private) + namespace role (organizations use
  ...
  `docs/organization-design.md`). Authorization is evaluated when signed URLs are issued (the
...
- Namespace profiles: `PATCH /api/v1/me/profile` (display name, bio, website, avatar URL) can
  ...
  `docs/namespace-design.md` §5.3-5.4)
...
See `docs/organization-design.md` for organization feature details, and
`docs/namespace-design.md` for namespaces (username = personal namespace, organization ID =
...
- **`api` runs on Cloud Run** (not GKE Autopilot + StatefulSet + PD). The earlier design was
  ...
  an option," but the Continuity migration (`docs/continuity-design.md`) moved git's source of
  ...
  `docs/continuity-design.md` §12 and `infra/README.md` for configuration details and rationale
...
- Cloud SQL uses Private IP only, reached directly from the `api` / `compact` Job via Direct VPC
  ...
  for the reasoning)
- WAL compaction (`docs/continuity-design.md` §10) is implemented as a Cloud Run Job, triggered
...
- Backing up git data: the WAL's `index.json` keeps old generations around via the bucket's
  ...
  (`docs/content-addressed-storage-design.md`), so across the whole bucket the same property
...
| **5. Operations** | Organizations/roles, search improvements, Terraform, backups, E2E compatibility test CI | Terraform and E2E tests are written but not yet applied. Organizations have only the DB/store skeleton — API and UI are not yet implemented (the full design is in `docs/organization-design.md`) |
| **6. Namespace unification** | Unify username = personal namespace and organization ID = organization namespace into a single `/{ns}` (with `/orgs/{name}` redirecting), and give users a profile too (display name, bio, website, avatar URL). Includes making reserved names a single source of truth, plus the HF-compatible `users/{u}/overview` / `organizations/{o}/overview` | Design is in `docs/namespace-design.md` (phase breakdown in the same doc's §13) |
