...
func runCompact(ctx context.Context, db *store.Store, obj storage.Storage, cfg *config.Config, args []string) error {
	...
	return forEachRepo(ctx, db, func(ref store.RepoRef) error {
		...
		if res.SweepDeferred {
			// Invariant 3 of §5, and the reason this is not one pass: see
			// wal.CompactAndSweep.
			slog.Info("compact: snapshot published, deferring the orphan sweep to the next run",
				"repo", refName(ref))
		}
		if len(res.Deleted) > 0 {
			slog.Info("compact: collected orphaned packs", "repo", refName(ref), "count", len(res.Deleted))
		}
		return nil
	})
}
