...
func TestIntegrationSyncJobs(t *testing.T) {
	forEachBackend(t, func(t *testing.T, s *Store) {
		...
		// A worker that lost its claim cannot write the outcome. This is what
		// keeps the lease from closing only half the race: without the fencing
		// token the straggler's write clears the 'running' row the new holder
		// is working under, and ClaimSyncJob's NOT EXISTS would then let a
		// third worker start on the same ref alongside it.
		r4 := f.repo(t, "alice", "fenced", "dataset", nil)
		if err := s.EnqueueSync(ctx, r4.ID, "main", "", "f1"); err != nil {
			t.Fatal(err)
		}
		stale, err := s.ClaimSyncJob(ctx, -time.Second) // lease already expired
		if err != nil || stale == nil {
			t.Fatalf("claim to be fenced = %+v, %v", stale, err)
		}
		if n, err := s.RequeueExpiredSyncJobs(ctx); err != nil || n != 1 {
			t.Fatalf("sweep the expired claim = %d, %v", n, err)
		}
		holder, err := s.ClaimSyncJob(ctx, testLease)
		if err != nil || holder == nil || holder.ID != stale.ID ||
			holder.Attempts != stale.Attempts+1 {
		...
		if fencedOut != nil {
			t.Fatalf("the stale finish released the ref: claimed %+v while %d still holds it",
				fencedOut, holder.ID)
		}
		...
		if n, _ := s.PendingSyncCount(ctx, r4.ID); n != 0 {
			t.Fatalf("PendingSyncCount after the real holder finished = %d", n)
		}
